...
func configure(status: Dictionary, status_data: Dictionary) -> void:
	...
	var name = String(data.get("name", status_id))
	...
	name_label.text = _status_label(name, status)
...
func _status_label(name: String, status: Dictionary) -> String:
	var status_id = String(status.get("id", ""))
	if status_id == "burn":
		return "%s x%d" % [name, int(status.get("layers", 1))]
	if status_id == "poison":
		return "%s %d" % [name, int(status.get("duration", 1))]
	return name
