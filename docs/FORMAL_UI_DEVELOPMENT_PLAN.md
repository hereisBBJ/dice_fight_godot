...
- 冻结第一版正式 UI 美术方向。
- 确定 UI 缩放目标，首版固定为 16:9 桌面窗口、1920x1080 构图。
- 确定 UI 美术资源命名规则。
...
- 主目标：固定 16:9 桌面窗口，`1920x1080` 构图。
- 首版最低人工检查分辨率：`1366x768`。
- 常见降级检查分辨率：`1600x900`。
- 首版不承诺移动端竖屏布局；项目窗口固定为 `1920x1080`，小窗口/缩放支持仅作为后续响应式扩展考虑。
...
- 2026-05-26 限制 `SkillButton` 技能图标显示范围：图标框固定为 44x44，图标纹理使用 `EXPAND_IGNORE_SIZE` 和 `STRETCH_KEEP_ASPECT_CENTERED`，避免 SVG 原始尺寸撑大技能按钮。
- 2026-05-26 修复技能按键局部重排后的节点路径和比例问题：`SkillButton` 脚本按节点名查找实际 Label 与图标节点，`SkillMargin` 不再使用非等比 `scale` 做布局，避免信息不刷新和图标变形。
...
| 6. 技能卡组件 | `[done]` | Codex + 子智能体 Franklin | 2026-05-26 | 已将 `SkillButton` 改为横向技能卡，展示需求、费用、模式和禁用原因，并限制 SVG 图标不会撑大按钮；`use_skill` payload 未变。 |
